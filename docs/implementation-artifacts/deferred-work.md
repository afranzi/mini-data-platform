...
## Deferred from: Story 3.1 (clean-state reproducibility) → hardening follow-ups

- **[Low → follow-up] Clean-apply ingress-nginx readiness race.** On a from-scratch apply, `module.argocd`'s helm release creates an Ingress that calls the `ingress-nginx` admission webhook before the controller is Ready → `failed calling webhook ... connection refused`; succeeds on retry. Harden with an explicit wait for the ingress-nginx controller (e.g. a `kubernetes_*`/`time_sleep` dependency, or `helm_release` `wait`/`depends_on`) so the first apply is clean. Documented as a retry step in the runbook for now.
- **[Low → follow-up] Destroy project-delete race.** `terraform destroy` removes the `argocd_application` resources from state instantly, but ArgoCD cascade-deletes them asynchronously, so deleting `argocd_project` fails ("referenced by N applications") on the first pass; succeeds on re-run. Harden with an explicit `depends_on` / wait between the project and the applications on destroy, or document the two-pass destroy (done in the runbook).
- **[Resolved in 3.1] ArgoCD auto-reconcile.** Apps now self-sync (automated `syncPolicy` on the application module); no manual `kubectl patch operation` needed (the 2.7 gap). Verified on a clean rebuild.
